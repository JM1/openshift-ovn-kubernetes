...
func ConfigureOVS(ctx context.Context, namespace, podName, hostIfaceName string,
	ifInfo *PodInterfaceInfo, sandboxID, deviceID string, getter PodInfoGetter, dpType string) error {
	...
	klog.Infof("ConfigureOVS: namespace: %s, podName: %s, network: %s, NAD %s, SandboxID: %q, PCI device ID: %s, UID: %q, MAC: %s, IPs: %v, Dp: %s",
		namespace, podName, ifInfo.NetName, ifInfo.NADName, sandboxID, deviceID, initialPodUID, ifInfo.MAC, ipStrs, dpType)
...
func (pr *PodRequest) ConfigureInterface(getter PodInfoGetter, ifInfo *PodInterfaceInfo) ([]*current.Interface, error) {
	...
	if !ifInfo.IsDPUHostMode {
		err = ConfigureOVS(pr.ctx, pr.PodNamespace, pr.PodName, hostIfName, ifInfo, pr.SandboxID, pr.CNIConf.DeviceID, getter, dpType)
		if err == nil && dpType == types.DatapathUserspace {
		...
		if err != nil {
			pr.deletePorts(hostIfName, pr.PodNamespace, pr.PodName, dpType)
			return nil, err
		}
	}
...
func (pr *PodRequest) deletePorts(ifaceName, podNamespace, podName string, dpType string) {
